...
export default function Sidebar({ user, activeCategory = "All", onCategoryChange, recentAgents = [] }: SidebarProps) {
  ...
  return (
    <>
      <style>{`
        ...
        .sb { width:240px; min-height:100vh; background:var(--sb-bg); border-right:0.5px solid var(--sb-border); display:flex; flex-direction:column; font-family:'DM Sans',sans-serif; flex-shrink:0; }
        ...
        .sb-icon-btn { width:30px; height:30px; background:transparent; border:none; border-radius:7px; display:flex; align-items:center; justify-content:center; cursor:pointer; color:var(--sb-muted); }
        ...
        .sb-upgrade { margin-left:auto; font-size:10px; font-weight:600; padding:2px 8px; border-radius:20px; background:transparent; border:0.5px solid var(--sb-accent2); color:var(--sb-accent); font-family:'Syne',sans-serif; }
      ...
      <aside className="sb">
        ...
        <nav className="sb-nav">
          {navItems.map((item, i) => (
            <button key={i} className={`sb-nav-item${activeNav === i ? " active" : ""}`} onClick={() => setActiveNav(i)}>
              <NavIcon index={i} />
              {item.label}
