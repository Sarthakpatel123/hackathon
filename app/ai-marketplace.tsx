...
function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const savedUser = localStorage.getItem("ai_marketplace_user");
    if (savedUser) {
  ...
  const saveUser = (userData: User | null) => {
    if (userData) {
      localStorage.setItem("ai_marketplace_user", JSON.stringify(userData));
    } else {
  ...
  const login = async (email: string, password: string): Promise<boolean> => {
    const users = JSON.parse(localStorage.getItem("ai_marketplace_users") || "[]");
  ...
  const signup = async (name: string, email: string, password: string): Promise<boolean> => {
    ...
    users.push(newUser);
    localStorage.setItem("ai_marketplace_users", JSON.stringify(users));
  ...
  const incrementQueries = () => {
    if (user) {
      updateUser({ totalQueries: user.totalQueries + 1 });
    }
  };

  return (
    <AuthContext.Provider value={{ user, login, signup, logout, updateUser, addToFavorites, removeFromFavorites, incrementQueries }}>
      {children}
...
function AuthModal({ isOpen, onClose, onSuccess }: { isOpen: boolean; onClose: () => void; onSuccess: () => void }) {
  ...
  return (
    <div
      style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 2000, padding: 20, animation: "fadeIn 0.2s ease" }}
      onClick={(e) => e.target === e.currentTarget && onClose()}
    >
      <div style={{ maxWidth: 440, width: "100%", background: "linear-gradient(135deg, #0f0f14 0%, #0a0a0f 100%)", borderRadius: 24, border: "1px solid rgba(255,255,255,0.1)", padding: 32, animation: "slideUp 0.3s ease" }}>
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.5)" }}>
            {isLogin ? "Login to access all agents" : "Join the AI Agents Marketplace"}
          </p>
        </div>
...
const AGENTS: Agent[] = [
  {
    id: "career",
    name: "Career Mitra",
    tagline: "Career guidance, skill development & job opportunities",
    category: "Career",
    icon: "🎯",
    color: "#ff6b35",
    bg: "#1a0d08",
    ...
    runs: 2150,
    featured: true,
    systemPrompt: `You are Career Mitra. Keep responses SHORT (max 3-4 sentences). 
  Be direct, practical, and actionable. No fluff, no long explanations.
  Use bullet points only when necessary. Get straight to the point.`,
    examples: [
    ...
    inputLabel: "Your career question",
    inputPlaceholder: "Ask about careers, skills, jobs, education...",
    btnLabel: "🎯 Ask Career Mitra",
    disclaimer: "Career guidance only — decisions should be based on your personal research and circumstances",
    quickRef: [
      { label: "NCS Portal", val: "ncs.gov.in" },
      { label: "PMKVY", val: "Skill India" },
      { label: "SWAYAM", val: "Free courses" },
    ],
  },
  {
    id: "krishi",
    name: "Krishi Mitra",
    tagline: "Crop advice, mandi prices & Govt schemes",
    ...
    accent: "#a5d6a7",
    muted: "#66bb6a",
    dim: "rgba(76,175,80,0.1)",
    runs: 3240,
    featured: true,
    systemPrompt: `You are Krishi Mitra. Give VERY CONCISE answers (2-3 sentences max).
  Just the key facts - crop advice, prices, schemes. No lengthy explanations.`,
    examples: [
    ...
    inputLabel: "Your farming question",
    inputPlaceholder: "Ask about crops, prices, schemes...",
    btnLabel: "🌱 Ask Krishi Mitra",
    disclaimer: null,
    quickRef: null,
  },
  {
    id: "gst",
    name: "GST Saathi",
    tagline: "Filing help, ITC & return status",
    category: "Tax",
    icon: "🧾",
    color: "#1565c0",
    bg: "#050d1a",
    ...
    accent: "#90caf9",
    muted: "#42a5f5",
    dim: "rgba(21,101,192,0.1)",
    runs: 2180,
    featured: false,
    systemPrompt: `You are GST Saathi. Keep answers BRIEF (1-2 short paragraphs max).
  Just deadlines, rates, and steps. No extra details unless asked.`,
    examples: [
    ...
    inputLabel: "Your GST question",
    inputPlaceholder: "Ask about filing, ITC, HSN codes...",
    btnLabel: "🧾 Ask GST Saathi",
    disclaimer: "Not legal advice — consult a CA for complex matters",
    quickRef: [
      { label: "GSTR-1", val: "11th monthly" },
      { label: "GSTR-3B", val: "20th monthly" },
      { label: "GSTR-9", val: "31st Dec" },
    ],
  },
  {
    id: "health",
    name: "Health Guide",
    tagline: "Symptoms, medicines & wellness advice",
    category: "Healthcare",
    icon: "⚕",
    color: "#c62828",
    bg: "#130808",
    ...
    accent: "#ef9a9a",
    muted: "#e57373",
    dim: "rgba(198,40,40,0.1)",
    runs: 4120,
    featured: true,
    systemPrompt: `You are Health Guide. Give SHORT, helpful answers (3 sentences max).
  Just symptoms info, basic guidance. Always add "see a doctor" briefly.`,
    examples: [
    ...
    inputLabel: "Your health question",
    inputPlaceholder: "Ask about symptoms, medicines, wellness...",
    btnLabel: "⚕ Ask Health Guide",
    disclaimer: "For information only — always consult a qualified doctor for medical advice",
    quickRef: null,
  },
  {
    id: "legal",
    name: "Kanoon Mitra",
    tagline: "Indian law made simple",
    category: "Legal",
    icon: "⚖",
    color: "#4a148c",
    bg: "#0c0714",
    border: "rgba(74,20,140,0.35)",
    accent: "#ce93d8",
    muted: "#ab47bc",
    dim: "rgba(74,20,140,0.12)",
    runs: 1560,
    featured: false,
    systemPrompt: `You are Kanoon Mitra. Keep it SHORT (2-3 sentences).
  Just the legal point, section number, and next step. No long explanations.`,
    examples: [
    ...
    inputLabel: "Your legal question",
    inputPlaceholder: "Ask about your rights, procedures, laws...",
    btnLabel: "⚖ Ask Kanoon Mitra",
    disclaimer: "General legal information only — consult a qualified advocate for your specific case",
    quickRef: null,
  },
  {
    id: "finance",
    name: "Artha Advisor",
    tagline: "Personal finance, loans & investments",
    category: "Finance",
    icon: "₹",
    color: "#e65100",
    bg: "#110900",
    ...
    accent: "#ffcc80",
    muted: "#ffa726",
    dim: "rgba(230,81,0,0.1)",
    runs: 2890,
    featured: false,
    systemPrompt: `You are Artha Advisor. Be CONCISE (max 50 words).
  Give numbers, rates, and simple advice. No financial jargon essays.`,
    examples: [
    ...
    inputLabel: "Your finance question",
    inputPlaceholder: "Ask about savings, investments, loans...",
    btnLabel: "₹ Ask Artha Advisor",
    disclaimer: "General financial education — consult a SEBI-registered advisor before investing",
    quickRef: null,
  },
];
...
function AgentPanel({ agent, onClose }: { agent: Agent; onClose: () => void }) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem(`chat_${agent.id}_${user?.id || "guest"}`);
    if (saved) setMessages(JSON.parse(saved));
  }, [agent.id, user?.id]);

  useEffect(() => {
    if (messages.length > 0) {
      localStorage.setItem(`chat_${agent.id}_${user?.id || "guest"}`, JSON.stringify(messages));
    }
  }, [messages, agent.id, user?.id]);
  ...
  function clearHistory() {
    if (confirm("Clear all messages?")) {
      setMessages([]);
      localStorage.removeItem(`chat_${agent.id}_${user?.id || "guest"}`);
    }
  }
      ...
      <div style={{ width: "100%", maxWidth: 900, background: agent.bg, borderRadius: 24, border: `1px solid ${agent.border}`, overflow: "hidden", display: "flex", flexDirection: "column", maxHeight: "90vh", boxShadow: `0 25px 50px -12px rgba(0,0,0,0.5)` }}>
        ...
        <div style={{ padding: "18px 24px", borderBottom: `1px solid ${agent.border}`, background: `linear-gradient(135deg, ${agent.dim} 0%, ${agent.bg} 100%)`, display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            {user && (
              <button
                onClick={() => isFavorite ? removeFromFavorites(agent.id) : addToFavorites(agent.id)}
            ...
            {messages.length > 0 && (
              <>
                ...
                <button onClick={clearHistory} style={{ fontSize: 12, padding: "6px 12px", borderRadius: 8, border: `1px solid ${agent.border}`, background: "rgba(255,255,255,0.05)", color: agent.muted, cursor: "pointer" }}>🗑️ Clear</button>
              </>
            )}
            ...
            <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: msg.role === "user" ? "flex-end" : "flex-start", gap: 6 }}>
              <div style={{ display: "flex", gap: 12, alignItems: "flex-start", maxWidth: "80%" }}>
                ...
                <div style={{ padding: "12px 16px", borderRadius: msg.role === "user" ? "16px 4px 16px 16px" : "4px 16px 16px 16px", background: msg.role === "user" ? agent.color : agent.dim, border: `1px solid ${agent.border}` }}>
                  {msg.role === "user" ? <p style={{ margin: 0, fontSize: 14, color: "#fff" }}>{msg.content}</p> : <>{renderText(msg.content)}<button onClick={() => navigator.clipboard.writeText(msg.content)} style={{ marginTop: 12, fontSize: 11, padding: "4px 12px", borderRadius: 6, border: `1px solid ${agent.border}`, background: "transparent", color: agent.muted, cursor: "pointer" }}>📋 Copy</button></>}
                </div>
              </div>
            </div>
          ))}
        ...
        <div style={{ padding: "16px 20px", borderTop: `1px solid ${agent.border}`, background: agent.dim, flexShrink: 0 }}>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "flex-end" }}>
            ...
            <textarea value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); runAgent(); } }} placeholder={agent.inputPlaceholder} rows={1} style={{ flex: 1, background: "rgba(255,255,255,0.03)", border: `1px solid ${agent.border}`, borderRadius: 12, color: agent.accent, fontFamily: "inherit", fontSize: 14, padding: "11px 14px", resize: "none", outline: "none" }} />
            ...
            <button onClick={runAgent} disabled={loading || (!query.trim() && !attachedFile)} style={{ padding: "11px 24px", borderRadius: 12, border: "none", background: loading || (!query.trim() && !attachedFile) ? agent.dim : `linear-gradient(135deg, ${agent.color} 0%, ${agent.color}cc 100%)`, color: loading || (!query.trim() && !attachedFile) ? agent.muted : "#fff", fontFamily: "inherit", fontSize: 14, fontWeight: 600, cursor: loading || (!query.trim() && !attachedFile) ? "not-allowed" : "pointer" }}>{loading ? "..." : "Send →"}</button>
          </div>
        </div>
    ...
    <div style={{ fontFamily: "'Inter', 'Syne', sans-serif", background: "radial-gradient(circle at 0% 0%, #0a0a0f 0%, #050508 100%)", minHeight: "100vh", color: "#f0eeff" }}>
      ...
      <div style={{ textAlign: "center", padding: "64px 24px 48px", position: "relative" }}>
        ...
        <div style={{ display: "flex", gap: 32, justifyContent: "center", marginBottom: 40, flexWrap: "wrap" }}>
          <div style={{ background: "rgba(255,255,255,0.03)", padding: "12px 24px", borderRadius: 40, border: "1px solid rgba(255,255,255,0.08)" }}>
            <div style={{ fontSize: 28, fontWeight: 800 }}>{AGENTS.length}</div>
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.45)" }}>🤖 Agents</div>
          </div>
          <div style={{ background: "rgba(255,255,255,0.03)", padding: "12px 24px", borderRadius: 40, border: "1px solid rgba(255,255,255,0.08)" }}>
            <div style={{ fontSize: 28, fontWeight: 800 }}>{totalRuns.toLocaleString()}</div>
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.45)" }}>⚡ Total Runs</div>
          </div>
          <div style={{ background: "rgba(255,255,255,0.03)", padding: "12px 24px", borderRadius: 40, border: "1px solid rgba(255,255,255,0.08)" }}>
            <div style={{ fontSize: 28, fontWeight: 800 }}>{CATEGORIES.length - 1}</div>
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.45)" }}>🎯 Categories</div>
          </div>
        </div>
